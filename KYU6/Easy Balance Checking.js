function balance(book) {
  let linebook=book.split('\n');
  let balance=linebook[0].replaceAll('!','').replaceAll('=','');
  let transaction=linebook.slice(1).
  filter(a=>''!=a).
  map(b=>b.split(' ')).
  map(c=>[c[0],c[1],c[2].replaceAll('!','').replaceAll('=','').replaceAll(':','')]);
  let total=transaction.reduce((a,b)=>a+Number(b[2]),0)
  let average=total/transaction.length;
